...
const sortBt = document.getElementById("sort-bt");
...
function displayPetData(petData) {
  const modalBx = document.querySelector(".pet-details-container");
  modalBx.innerHTML = `
  <div>
    <img src="${
      petData.image
...
function displayCategoryBtn(categories) {
  ...
  categories.forEach((category) => {
    const categoryBt = document.createElement("button");
    categoryBt.classList.add(
      "btn",
      "btn-ghost",
      "flex",
      "border-gray-300",
    ...
    categoryBt.innerHTML = `
    <img src=${category.category_icon} alt="" class="w-7 h-7">
    <div class="font-bold text-lg font-inter">${category.category}</div>`;
    categoryContainer.appendChild(categoryBt);
    categoryBt.addEventListener("click", () => {
      const categoryBts = document.querySelectorAll(".category-btn");
      categoryBts.forEach((btn) => {
      ...
      categoryBt.classList.add("bg-[#0E7A81]");
      categoryBt.classList.add("text-white");
      petCardContainer.innerHTML = "";
...
function displayLikedPet(petsImage) {
  ...
  imgDiv.classList.add("rounded-md", "h-[150px]", "xl:h-[100px]");
  imgDiv.innerHTML = `<img src="${petsImage}" alt="Liked Pet Image" class="rounded-md h-full object-cover w-full"/>`;
...
function CountDownModal(petId) {
  ...
  const adoptBn = document.querySelector("#adopt-btn");
